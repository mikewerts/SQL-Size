...
(
	...
	,butrend.Trend
	 ...
	 insert into #Last_12_Month_Backup_trend_forecast
	   ...
	   select joinbutrend.MonthsAgo - 12 as [month], @DatabaseName as [DatabaseName], joinbutrend.AvgSizeMB, -- Trend --from joinbutrend
...
with percent_annual as
(
select DatabaseName, MonthsAgo, AvgSizeMB, Trend , LAG(AvgSizeMB, 13, 0) over(order by DatabaseName)  as 'init_size'
 
from #Last_12_Month_Backup_trend_forecast
--where MonthsAgo in (0,12) --12
...
select DatabaseName, AvgSizeMB, 0 as Trend  --, LAG(AvgSizeMB, 13, 0) over(order by DatabaseName)  as 'init_size'
--* 
from #Last_12_Month_Backup_trend_forecast tf1
where MonthsAgo = 0 --12

union 

select DatabaseName, 0 as AvgSizeMB, Trend  --, LAG(AvgSizeMB, 13, 0) over(order by DatabaseName)  as 'init_size'
--* 
from #Last_12_Month_Backup_trend_forecast tf1
where MonthsAgo = 12
),
...
select DatabaseName, SUM(AvgSizeMB) as init_size, SUM(Trend) as  Trend from e
group by DatabaseName
)

select DatabaseName, 'Percent Gain' =
case when init_size < Trend
/* gain */
then init_size/Trend

when Trend > init_size 
/* loss */
then (-1 * (Trend/init_size))
else
/* unch */
0
end
