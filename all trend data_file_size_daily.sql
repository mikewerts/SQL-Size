...
--select *  from Last_12_Month_BU_Ave
...
/* first, declare a cursor for each server */
/*
declare servcursor
for 
select distinct server_name from #Last_12_Month_Backup
	open servcursor

	 fetch next from servcursor
	 into @servername
...
--select * from db_trend


/* Use the following for known positive trendlines 
where PVT.[endpoint] = 'DEV-ITSQL-01\VINYL'
and PVT.[database_name] =	'UnderwritingReviewSheet' */
...
--select * from #12_Month_Forecast_log
/* Use the following for known positive trendlines 
where PVT.[endpoint] = 'DEV-ITSQL-01\VINYL'
and PVT.[database_name] =	'UnderwritingReviewSheet' */
...
--select * from #Last_12_Month_log
select * from #12_Month_Forecast_log
--select * from #12_Before_12_After_log 
...
;with get_log_new as(
 ...
 ,max([12ba].[-11]) as 'Size 12 Months Ago (MB)'
  ...
  ,max(ggt.database_size) as 'Current Size (MB)'

 
	,case when (max([12ba].[0]) < max(ggt.database_size))
	/* trendline lags behind current DB size */
	then max(cast((ggt.database_size) * (1 + ([ggt].slope -1) /2) as int))
	else 
	/* use trendline */
	...
	case when (max([12ba].[0]) < max(ggt.database_size))
	/* trendline lags behind current DB size */
	then max(cast((ggt.database_size) * [ggt].slope as int))
	else 
	/* use trendline */
...
--  join [master].[sys].[master_files] s on db_name(s.database_id) = DatabaseName 
  ...
  join #Get_group_trend [ggt] on  [ggt].[database_name] = [12ba].[database_name] and [ggt].[Endpoint] = [12ba].[Endpoint]
...
--select * from #12_Before_12_After [12ba]
...
--  join [master].[sys].[master_files] s on db_name(s.database_id) = DatabaseName 
  ...
  join #Get_group_log_trend [ggt] on  [ggt].[database_name] = [12ba].[database_name] and [ggt].[Endpoint] = [12ba].[Endpoint]
 ...
 insert into #get_log_new select *  from get_log_new
