...
;with growth_by_volume as
 (
	...
	,max(cast(gd.[database_size] as int)) as 'Current Database Size' 
	...
	--,max(cast(gdn.[Projected Size 6 Months (MB)] as int)) as 'Total Projected Size 6 Months (MB)'
	--,max(cast(gdn.[Projected Size 12 Months (MB)] as int)) as 'Total Projected Size 12 Months (MB)'

	from  #get_database_new gdn
	
 join #Get_datafiles gd on gd.Endpoint = gdn.Endpoint and gd.database_name = gdn.DatabaseName
...
-- where gdn.DatabaseName = (select distinct db_name(database_id) from #Get_datafiles) gd
 ...
 group by gd.[Endpoint], gd.volume_mount_point
...
-- join #Get_datafiles gd on db_name(gd.database_id) = gdn.DatabaseName 
 join #Get_logfiles gl on gln.DatabaseName = gl.database_name and gln.Endpoint = gl.Endpoint
 group by gln.[Endpoint], gl.volume_mount_point
 ...
 [Endpoint]
 ,[Volume Mount Point]
 , max([Current Database Size]) as 'Current Database Size', 
 max([Projected Database Size 6 Months (MB)]) as 'Projected Database Size 6 Months (MB)',
max([Projected Database Size 12 Months (MB)]) as 'Projected Database Size 12 Months (MB)', 
max([Total Log File Size (current)]) as 'Current Log Size',
max([Projected Log Size 6 Months (MB)])  as 'Projected Log Size (6 Months)',
max([Projected Log Size 12 Months (MB)])  as 'Projected Log Size (12 Months)',
max([Projected Database Size 6 Months (MB)]) + max([Projected Log Size 6 Months (MB)]) as 'Total Projected Size (6 Months)',
max([Projected Database Size 12 Months (MB)]) + max([Projected Log Size 12 Months (MB)]) as 'Total Projected Size (12 Months)',
max([Available MB]) as 'Available MB',
max([Available MB]) - (max([Projected Log Size 6 Months (MB)])  -  max([Projected Database Size 6 Months (MB)])) - (max([Current Database Size])  - max([Total Log File Size (current)]))  as 'Available MB (6 Months)'   ,
max([Available MB]) - (max([Projected Log Size 12 Months (MB)]) - max([Projected Database Size 12 Months (MB)])) - (max([Current Database Size])  - max([Total Log File Size (current)])) as 'Available MB (12 Months)'



from growth_by_volume
/* exclude Rubrik mount points */
where [Volume Mount Point] not like '\\%'
--where  [Endpoint]  = 'UAT-SQLCOMPX'

--and [Database_Name] = 'UAT_WCXP'

group by [Endpoint], [Volume Mount Point]
order by [Endpoint], [Volume Mount Point]
--)
